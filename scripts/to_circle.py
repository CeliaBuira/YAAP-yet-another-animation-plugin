...
@qgsfunction(group='Custom', referenced_columns=[])
def to_circle_optimized(geometry: QgsGeometry, t: float, centroid, goal):
    """
    Calculates the sum of the two parameters value1 and value2.
    <h2>Example usage:</h2>
    <ul>
    ...
    #return QgsGeometry.fromPolygonXY([[QgsPointXY(vector.toPointF()) for vector in vectors]])
    
    #vectors = [QVector2D(v.toQPointF()) for v in vectors.vertices()]
    vectors = [vector * lerp(original_dist, goal, t) for vector, original_dist in zip(vectors, distance_centroid_vertex)]
    end_vertices = [QgsPointXY(centroid.asQPointF() + final_vector.toPointF()) for final_vector in vectors]
